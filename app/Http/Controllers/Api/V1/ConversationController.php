...
class ConversationController extends Controller
{
    public function messages_store(Request $request)
    {
        ...
        if($conv->sender_type == 'vendor' && $conversation->sender){
            ...
            $order = Order::where('user_id',$request->user()->id)->where('store_id', $vd->stores[0]->id)->statusSearch( ['pending','accepted','confirmed','processing','handover','picked_up'])->count();
            ...
            $order = Order::where('user_id',$request->user()->id)->where('delivery_man_id', $user2->id)->statusSearch( ['pending','accepted','confirmed','processing','handover','picked_up'])->count();
    ...
    public function messages(Request $request)
    {
        ...
        if(isset($conversation)){
            if($conversation->sender_type == 'vendor' && $conversation->sender){
                ...
                $order = Order::where('user_id',$user->user_id)->where('store_id', $vd->stores[0]->id)->statusSearch( ['pending','accepted','confirmed','processing','handover','picked_up'])->count();
                ...
                $order = Order::where('user_id',$user->user_id)->where('delivery_man_id', $user2->id)->statusSearch( ['pending','accepted','confirmed','processing','handover','picked_up'])->count();
    ...
    public function dm_messages_store(Request $request)
    {
        ...
        if($conv->sender_type == 'vendor' && $conversation->sender){
            $vd = Vendor::find($conv->sender->vendor_id);
            $order = Order::where('delivery_man_id',$dm->id)->where('store_id', $vd->stores[0]->id)->statusSearch( ['pending','accepted','confirmed','processing','handover','picked_up'])->count();
        }else if($conv->receiver_type == 'vendor' && $conversation->receiver){
            $vd = Vendor::find($conv->receiver->vendor_id);
            $order = Order::where('delivery_man_id',$dm->id)->where('store_id', $vd->stores[0]->id)->statusSearch( ['pending','accepted','confirmed','processing','handover','picked_up'])->count();
        }else if($conv->sender_type == 'customer' && $conversation->sender){
            $user = User::find($conv->sender->user_id);
            $order = Order::where('delivery_man_id',$dm->id)->where('user_id', $user->id)->statusSearch( ['pending','accepted','confirmed','processing','handover','picked_up'])->count();
