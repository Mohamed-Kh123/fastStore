...
class VendorController extends Controller
{
    ...
    public function get_order_details(Request $request)
    {
        ...
        $vendor = $request['vendor'];
        $store_ids = $vendor->stores ? (optional($vendor->stores)->pluck('id')->toArray() ?? []) : [];
        ...
        $details = OrderDetail::where('order_id', $order->id)->whereIn('item_details->store_id', $store_ids)->get();
...
//        $details = isset($order->details)?$order->details:null;
    ...
    public function get_all_orders(Request $request)
    {
         ...
         */
        $store_ids = $vendor->stores ? (optional($vendor->stores)->pluck('id')->toArray() ?? []) : [];
        $orders = Order::whereHas('details', function ($query) use ($store_ids) {
            $query->whereIn('item_details->store_id', $store_ids);
        })
            ->with('customer')
            ->Notpos()
            ->NotDigitalOrder()
